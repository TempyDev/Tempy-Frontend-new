      ...
      >
        ...
        {mobileOpen && (
          <div className="md:hidden border-t border-slate-200 bg-white px-6 py-4 space-y-3">
            ...
            <MobileLink label="How it works" />
            <MobileLink label="Help" />
  ...
  return (
    <NavLink
      ...
      className={`transition ${
        active ? "text-purple-600 font-semibold" : "text-slate-700 hover:text-slate-900"
      }`}
    >
      {label}
    </NavLink>
...
function DesktopDropdown() {
  return (
    <div className="relative group">
      ...
      <div className="absolute left-1/2 top-full z-10 mt-3 w-44 -translate-x-1/2 hidden group-hover:block
...
function DropdownItem({ label }) {
  return (
    <li>
      <a
        href="#"
        className="block rounded p-2 hover:bg-neutral-tertiary-medium hover:text-heading transition"
      >
        {label}
      </a>
    </li>
...
function MobileLink({ label }) {
  return (
    <div className="rounded-lg px-3 py-2 font-medium hover:bg-slate-100">
      {label}
    </div>
  )
}
...
function Chevron({ rotate }) {
  return (
    <svg
      className={`h-4 w-4 transition ${rotate ? "rotate-180" : ""}`}
      fill="none"
      stroke="currentColor"
      strokeWidth="2"
      viewBox="0 0 24 24"
